...
epoller_( new Channel(this, weakupFd_)),
...
{
    quit_ = true;
    if(!isInLoopThread) wakeup();//触发wakeupFd_可读事件，防止epoll_wait()阻塞导致无法退出Loop循环
}
...
void EventLoop::queueInLoop(Fucntor cb)
{
...
{
    unit64_t data = 1;
    ssize_t ret = read(weakupFd_, &data, sizeof(data));
    if(ret !=szieof(data))
    {
        LOG_ERROR << "EventLoop::handleRead() reads" << res << "bytes instead of 8";
    }
}
...
//channel是否在epoller
bool EventLoop::hasChannel(Channnel *channel)
{
    return epoller_->hasChannel();
}

//在time时刻执行回调函数
void EventLoop::runAt(TimeStamp time, Functor &&cb)
{
    timerQueue_->addTime(std::move(cb));
}
